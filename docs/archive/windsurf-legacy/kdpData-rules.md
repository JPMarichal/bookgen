...
- **Archivo metadata.md**: `bios/x/output/kdp/metadata.md` completo con todos los campos
- **Archivo descripcion.md**: `bios/x/output/kdp/descripcion.md` con descripción de venta
...
1. **Cálculo del ancho del lomo** (requisito obligatorio):
   ...
   ```
   Guardar salida en: `bios/x/output/kdp/logs/spine-calculation-<fecha>.log`

2. **Verificación de archivos KDP**:
   ```bash
   ls -lh bios/x/output/kdp/ > bios/x/output/kdp/logs/files-check-<fecha>.log
   ```

3. **Conteo de palabras clave**:
   ```bash
   # Verificar que metadata.md contenga exactamente 15 keywords
   grep -c "keyword" bios/x/output/kdp/metadata.md
   ```
